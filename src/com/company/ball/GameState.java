...
public class GameState implements Serializable{
    ...
    public boolean gameOver , start , stopManu;
    private String playerName;
    private  int playersSuns;

    public enum type{ hard , normal}
    private   type type;
    ...
    public  void  randomZombie() throws Exception {
        ...
        if(random == 0) {
            zombies.add(new Zombie(Zombie.zombieType.NORMAL));
        }else  if(random == 1) {
            zombies.add(new Zombie(Zombie.zombieType.CONEHEAD));
        }else  if(random == 2) {
            zombies.add(new Zombie(Zombie.zombieType.BUCKHEAD));
        }
    }
    /**
    ...
    public void update() throws Exception {
        ...
        if (!stopManu) {
            ...
            while (Iter.hasNext()) {
                Zombie z = Iter.next();
                z.move();  // speed ro dorost con 88888888888888888888888888888888888888888888
                if (!z.isAlive()) {
                    Iter.remove();
                }
//
